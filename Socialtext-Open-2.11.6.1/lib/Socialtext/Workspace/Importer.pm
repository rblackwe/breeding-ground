...
use Readonly;
use Socialtext::Validate qw( validate FILE_TYPE BOOLEAN_TYPE );
...

{
    ...
    sub new {
        ...
        my ( $name, $version ) = $p{tarball} =~ /([\w-]+)(\.\d+)?\.tar/
            or die
        ...
        if ( $ws && ! $p{overwrite} ) {
            die "Cannot restore $name workspace, it already exists.";
        }
...
sub _workspace_info_file { $_[0]->{name} . '-info.yaml' }
...
sub _import_data_dirs {
    opendir my $dh, '.'
        or die "Cannot opendir on .: $!";

    for my $from ( grep { !/\./ && -d } readdir $dh ) {
        my $to = Socialtext::File::catdir(
            Socialtext::AppConfig->data_root_dir(), $from );

        next unless -d $to;

        Socialtext::File::Copy::Recursive::dircopy( $from, $to );
    }
}
...
sub _permissions_file { $_[0]->{name} . '-permissions.yaml' }
...
sub _users_file { $_[0]->{name} . '-users.yaml' }
