...
sub _data_dir_paths {
    ...
    return (
        ...
        Socialtext::Paths::plugin_directory( $self->name ),
        Socialtext::Paths::user_directory( $self->name ),
    );
}
...

{
    ...
    sub export_to_tarball {
        ...
        my @dirs = $self->_data_for_export();

        my $tarball = Socialtext::File::catfile( $tarball_dir,
            $self->name . '.' . $EXPORT_VERSION . '.tar' );

        my $data_root_dir = Socialtext::AppConfig->data_root_dir();

        # XXX - I'm afraid to use Archive::Tar here cause it will load
        # all the data into memory, which could be enormous for
        # workspaces with many pages and attachments.
        #
        # Also note that append does not work with the -z flag so we have
        # to gzip after all dirs/files are added to the tarball
        for my $dir (@dirs) {
            my $tar_opts = $dir eq $dirs[0] ? 'c' : 'r';
            $tar_opts .= 'f';

            if ( $dir =~ m{^\Q$data_root_dir\E/?} ) {
                local $CWD = $data_root_dir;
                $dir =~ s{^\Q$data_root_dir\E/?}{};

                system( 'tar', $tar_opts, $tarball, $dir )
                    and die "tarball creation for workspace export failed: $!";
            }
            else {
                local $CWD = $dir;
                system( 'tar', $tar_opts, $tarball, '.' )
                    and die "tarball creation for workspace export failed: $!";
            }
        }
        ...
        if ( my $logo_file = $self->logo_filename() ) {
            ...
            system( 'tar', 'rf', $tarball, File::Basename::basename($logo_file) )
                and die "tarball creation for workspace export failed: $!";
        }

        system( 'gzip', '-f', $tarball )
            and die "Cannot gzip $tarball: $!";
...
sub _data_for_export {
    ...
    my @dirs = $self->_data_dir_paths();

    my $tmpdir = File::Temp::tempdir( CLEANUP => 1 );
    $self->_dump_to_yaml_file($tmpdir);
    $self->_dump_users_to_yaml_file($tmpdir);
    $self->_dump_permissions_to_yaml_file($tmpdir);
...
sub _dump_to_yaml_file {
    ...
    my $file = Socialtext::File::catfile( $dir, $self->name . '-info.yaml' );
...
sub _dump_users_to_yaml_file {
    ...
    my $file = Socialtext::File::catfile( $dir, $self->name . '-users.yaml' );
...
sub _dump_permissions_to_yaml_file {
    ...
    my $file = Socialtext::File::catfile( $dir, $self->name . '-permissions.yaml' );
...
=head2 $workspace->export_to_tarball( dir => $dir )
